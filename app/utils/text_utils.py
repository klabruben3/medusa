...
# Only keep text blocks that plausibly relate to the course/module — drops
# generic boilerplate, orphaned list-markers ("a.", "b."), and greeting
# fragments that survived the language filter but add nothing useful for
# downstream parsing into a module object.
_RELEVANCE_KEYWORDS = {
    # module identity
    "module", "course", "code",
    # people / contact (ModuleGroup, lecturer/email/office)
    "lecturer", "tutor", "office", "consultation", "email", "contact",
    "group", "venue", "period",
    # assessments (AssessmentComponent)
    "test", "exam", "examination", "assessment", "assignment",
    "mark", "marks", "score", "weight", "%", "percent",
    "class-work", "practical", "aleks", "mcq", "attendance",
    "duration", "study unit", "drop", "lowest",
    # pass requirements / participation formula
    "participation", "admission", "pass", "fail", "minimum", "threshold",
    "completion",
    # dates / scheduling
    "semester", "recess", "deadline", "schedule", "date", "eFundi".lower(),
    "tutorial", "memorandum", "memo",
}

_MIN_ALPHA_CHARS = 8  # blocks shorter than this are almost never meaningful


def is_relevant_block(text: str) -> bool:
    text = text.strip()
    if _EMAIL_RE.search(text) or _PHONE_RE.search(text) or _TITLE_RE.search(text):
        return True  # contact info and named staff are always relevant

    if len(re.sub(r"[^a-zA-Z]", "", text)) < _MIN_ALPHA_CHARS:
        return False

    lowered = text.lower()
    return any(kw.lower() in lowered for kw in _RELEVANCE_KEYWORDS)


def filter_relevant_blocks(blocks):
    filtered = []
    for block in blocks:
        if block["type"] == "text" and not is_relevant_block(block["content"]):
            continue
